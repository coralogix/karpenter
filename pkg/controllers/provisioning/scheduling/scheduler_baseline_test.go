...
func TestNewSchedulerFromBaselineOwnsAttemptState(t *testing.T) {
	template := &NodeClaimTemplate{
		...
		InstanceTypeOptions: []*cloudprovider.InstanceType{{Name: "instance"}},
	}
	inputs := &NodePoolInputs{
	...
	firstUsage := first.daemonOverheadGroups[template][0].HostPortUsage
	secondUsage := second.daemonOverheadGroups[template][0].HostPortUsage
