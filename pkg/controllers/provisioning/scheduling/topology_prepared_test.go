...
func assertSparseInverseHostnameMatchesDense(t *testing.T, materialized *Topology, preparedGroup *TopologyGroup, groupHash uint64, incoming *corev1.Pod) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := preparedGroup.GetWithActiveDomains(incoming, podDomains, nodeDomains, materialized.activeHostnameDomains)
			want := denseGroup.Get(incoming, podDomains, nodeDomains)
