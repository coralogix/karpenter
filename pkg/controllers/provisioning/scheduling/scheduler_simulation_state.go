...
func NewPreparedSchedulerState(ctx context.Context, baseline *SchedulerBaseline, stateNodes []*state.StateNode, clocks ...clock.Clock) (*PreparedSchedulerState, error) {
	...
	for _, node := range stateNodes {
		...
		if node.MarkedForDeletion() {
			deletingNodeNames.Insert(node.Name())
			continue
		}
		taints := node.Taints()
		daemons := getCompatibleDaemonPods(ctx, node, taints, daemonSetPods)
		daemonResources := resources.RequestsForPods(daemons...)
		nodePool := nodePoolsByName[node.Labels()[v1.NodePoolLabelKey]]
		isUnderConsolidateAfter := baseline.options.enforceConsolidateAfter && disruption.IsUnderConsolidateAfter(nodePool, node.NodeClaim, clockSource)
		prepared := prepareExistingNode(node, taints, daemonResources, instanceTypeForNode(baseline.inputs, node), isUnderConsolidateAfter)
...
func (p *PreparedSchedulerState) NewScheduler(
	...
	s.instanceTypes = p.baseline.inputs.instanceTypes
	s.cachedResourceClaims = map[types.NamespacedName]*resourcev1.ResourceClaim{}
	s.deletingNodeNames = sets.New(p.deletingNodeNames.UnsortedList()...)
...
func instanceTypeForNode(inputs *NodePoolInputs, node *state.StateNode) *cloudprovider.InstanceType {
	instanceTypeName := node.Labels()[corev1.LabelInstanceTypeStable]
	nodePoolName := node.Labels()[v1.NodePoolLabelKey]
	if instanceTypeName == "" || nodePoolName == "" {
		return nil
	}
	for _, instanceType := range inputs.instanceTypes[nodePoolName] {
