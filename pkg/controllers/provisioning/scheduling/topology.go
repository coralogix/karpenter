...
func (t *Topology) countDomains(ctx context.Context, tg *TopologyGroup) error {
	...
	for _, n := range t.stateNodes {
		...
		if !tg.nodeFilter.Matches(n.Node.Spec.Taints, LabelRequirementsFor(t.nodeLabelRequirements, stateNodeCacheKey(n), n.Node.Labels)) {
			continue
		}
	...
	for i, p := range pods {
		...
		if previousNode != nil && previousNode.Name == p.Spec.NodeName {
			...
			nodeRequirements = LabelRequirementsFor(t.nodeLabelRequirements, coreNodeCacheKey(node), node.Labels)
