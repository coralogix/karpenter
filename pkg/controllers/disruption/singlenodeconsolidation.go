...
func (s *SingleNodeConsolidation) ComputeCommands(ctx context.Context, disruptionBudgetMapping map[string]int, candidates ...*Candidate) ([]Command, error) {
	...
	simulator, err := NewSchedulingSimulator(ctx, s.kubeClient, s.cluster, s.provisioner, candidates...)
	if err != nil {
		return []Command{}, err
	}
	...
	if !constrainedByBudgets && !constrainedByPace {
		// if there are no candidates because of a budget or pace limit, don't mark
