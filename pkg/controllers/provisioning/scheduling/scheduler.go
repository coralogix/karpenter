...
func newSchedulerFromBaseline(
	...
	s := newSchedulerWithBaseline(baseline, cluster, topology, recorder, clock, volumeSource,
		nodePoolRemainingResources(baseline.inputs))
...
func (s *Scheduler) addToExistingNode(ctx context.Context, pod *corev1.Pod) error {
	...
	volumes, err := s.volumeSource.Volumes(ctx, pod)
	if err != nil {
		return err
	}
