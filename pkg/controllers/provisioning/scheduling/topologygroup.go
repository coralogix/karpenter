...
func (t *TopologyGroup) GetWithActiveDomains(pod *corev1.Pod, podDomains, nodeDomains *scheduling.Requirement, activeDomains sets.Set[string]) *scheduling.Requirement {
	return t.get(pod, podDomains, nodeDomains, activeDomains)
}

func (t *TopologyGroup) get(pod *corev1.Pod, podDomains, nodeDomains *scheduling.Requirement, activeDomains sets.Set[string]) *scheduling.Requirement {
	switch t.Type {
	case TopologyTypeSpread:
	...
	case TopologyTypePodAntiAffinity:
		return t.nextDomainAntiAffinity(podDomains, nodeDomains, activeDomains)
	default:
