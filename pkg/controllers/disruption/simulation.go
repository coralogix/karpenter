...
func captureSimulationNodesAndPods(ctx context.Context, kubeClient client.Client, cluster *state.Cluster, provisioner *provisioning.Provisioner, clk clock.Clock, recorder events.Recorder) (state.StateNodes, []*corev1.Pod, []*corev1.Pod, pdb.Limits, error) {
	...
	deletingPods = lo.Filter(deletingPods, func(pod *corev1.Pod, _ int) bool {
		return pdbs.IsCurrentlyReschedulable(pod, clk, recorder)
	})
	return nodes, pendingPods, deletingPods, pdbs, nil
}

func captureSimulationCandidatePods(candidates []*Candidate, deletingNodes state.StateNodes, pdbs pdb.Limits, clk clock.Clock, recorder events.Recorder) (map[string][]*corev1.Pod, sets.Set[string], sets.Set[string], error) {
	candidatePods := map[string][]*corev1.Pod{}
...
func buildSchedulingSimulatorSnapshot(ctx context.Context, kubeClient client.Client, cluster *state.Cluster, provisioner *provisioning.Provisioner, clk clock.Clock, recorder events.Recorder, candidates ...*Candidate) (*schedulingSimulationSnapshot, error) {
	nodes, pendingPods, deletingPods, pdbs, err := captureSimulationNodesAndPods(ctx, kubeClient, cluster, provisioner, clk, recorder)
	if err != nil {
		return nil, err
	}
	candidatePods, candidateUniverse, deletingCandidates, err := captureSimulationCandidatePods(candidates, nodes.Deleting(), pdbs, clk, recorder)
	if err != nil {
		return nil, err
	}
...
func NewSchedulingSimulator(ctx context.Context, kubeClient client.Client, cluster *state.Cluster, provisioner *provisioning.Provisioner, clk clock.Clock, recorder events.Recorder, candidates ...*Candidate) (*SchedulingSimulator, error) {
	return newSchedulingSimulator(ctx, kubeClient, cluster, provisioner, clk, recorder, nil, candidates...)
}

func NewConsolidationSchedulingSimulator(ctx context.Context, kubeClient client.Client, cluster *state.Cluster, provisioner *provisioning.Provisioner, clk clock.Clock, recorder events.Recorder, candidates ...*Candidate) (*SchedulingSimulator, error) {
	return newSchedulingSimulator(ctx, kubeClient, cluster, provisioner, clk, recorder, []scheduling.Options{scheduling.IsConsolidationSimulation}, candidates...)
}
...
func newSchedulingSimulator(ctx context.Context, kubeClient client.Client, cluster *state.Cluster, provisioner *provisioning.Provisioner, clk clock.Clock, recorder events.Recorder, extraOpts []scheduling.Options, candidates ...*Candidate) (*SchedulingSimulator, error) {
	...
	inputs, err := provisioner.NewPreparedSimulationInputs(
		...
		allPods,
		snapshot.nodes,
		simulationSchedulerOptions(ctx, extraOpts...)...,
	)
	if err != nil {
...
func simulationSchedulerOptions(ctx context.Context, extraOpts ...scheduling.Options) []scheduling.Options {
...
func (simulator *SchedulingSimulator) Simulate(ctx context.Context, candidates ...*Candidate) (scheduling.Results, error) {
	...
	releasingIDs := make([]provisioning.SimulationPodID, 0, len(simulator.deletingNodePodIDs))
	releasingSeen := sets.New[provisioning.SimulationPodID]()
	appendReleasingIDs := func(values ...provisioning.SimulationPodID) {
		for _, id := range values {
			if releasingSeen.Has(id) {
				continue
			}
			releasingSeen.Insert(id)
			releasingIDs = append(releasingIDs, id)
		}
	}
	...
	for _, candidate := range candidates {
		candidateIDs := simulator.candidatePodIDs[candidate.Name()]
		appendIDs(candidateIDs...)
		appendReleasingIDs(candidateIDs...)
	}
	appendIDs(simulator.deletingNodePodIDs...)
	appendReleasingIDs(simulator.deletingNodePodIDs...)
