...
func BenchmarkSimulateScheduling_ClusterFixture(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if _, err := simulateSchedulingWithFactory(bench.ctx, bench.env.Client, bench.env.Cluster, bench.env.Provisioner, bench.env.Clock, bench.env.Recorder, bench.schedulerFactory, candidate); err != nil {
